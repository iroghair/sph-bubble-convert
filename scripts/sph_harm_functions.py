...
def sph_harm_fast(theta, phi, l_max):
    """
    Vectorized real spherical harmonics using scipy.special.sph_harm_y_all.
    Output shape: (num_samples, (l_max+1)^2)
    """

    # Method from scipy to compute all spherical harmonics up to degree l_max
    # Phi and theta are swapped w.r.t. the old method
    # The entry at (j, i) corresponds to degree j and order i for all 0 <= j <= n and -m <= i <= m.
    Y_complex = sp.special.sph_harm_y_all(l_max, l_max, phi, theta)

    N = len(theta)
    num_orbitals = (l_max + 1) ** 2
    out = np.zeros((num_orbitals, N))
    list_orbitals = np.arange(num_orbitals)
    ...
    for i, j in enumerate(list_orbitals):
        ...
        sh_complex = Y_complex[l, m]

        if m < 0:
            Y_lm = np.sqrt(2) * sh_complex.imag
        elif m == 0:
            Y_lm = sh_complex.real
        elif m > 0:
            Y_lm = np.sqrt(2) * sh_complex.real
